...
def crop_validation_images(dataset_path, side_length, stride, scales, validation_folder_name):
    ...
    for image in tqdm(images):
        if not os.path.exists(f'{validation_folder_name}/crop_images/{".".join(image.split(".")[:-1])}'):
            os.mkdir(f'{validation_folder_name}/crop_images/{".".join(image.split(".")[:-1])}')
        ...
        for i in range(len(scales)):
            ...
            if not os.path.exists(f'{validation_folder_name}/crop_images/{".".join(image.split(".")[:-1])}/{scales[i]}'):
                os.mkdir(f'{validation_folder_name}/crop_images/{".".join(image.split(".")[:-1])}/{scales[i]}')
            
            for j in range(len(scaled_im_list[i])):
                scaled_im_list[i][j].save(f'{validation_folder_name}/crop_images/{".".join(image.split(".")[:-1])}/{scales[i]}/{scaled_position_list[i][j]}.png')
