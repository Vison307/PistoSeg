...
def unormalize(tensor, mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD):
    tensor = tensor.clone()
...
class MosaicModule(pl.LightningModule):
    def __init__(self, args):
        ...
        if self.args.dataset == 'wsss4luad':
            self.train_dice = DiceLoss(mode='multiclass', ignore_index=self.args.num_classes)
        else:
            self.train_dice = DiceLoss(mode='multiclass')
        ...
        if args.tta:
            self.tta_wrapper = partial(tta.SegmentationTTAWrapper, transforms=tta.aliases.d4_transform(), merge_mode='mean')
    
        if 'Unet' in self.args.model:
    ...
    def training_step(self, batch, batch_idx):
        ...
        # self.log('train_ce', ce_mean, prog_bar=True)
    ...
    def validation_epoch_end(self, validation_step_outputs):
        if self.args.dataset == 'wsss4luad':
            ...
            for idx, (k, mask_pred) in enumerate(self.pred_big_mask_dict.items()):
                mask_pred /= self.cnt_big_mask_dict[k]
                mask = np.asarray(Image.open(os.path.join('/'.join(self.args.val_data.split('/')[:-1]), 'mask', k + '.png')))
                
                # visualize_dict = {
                #     f'{k}.png': np.asarray(Image.open(os.path.join('/'.join(self.args.val_data.split('/')[:-1]), 'img', k + '.png'))),
                #     'mask': mask,
                #     'mask_pred': np.argmax(mask_pred, axis=-1),
                # }
                # self.show_image_on_tensorboard('validation mask', self.current_epoch * 40 + idx, **visualize_dict)
            ...
            print('\n' + '-' * 50)
            print("\nExperiment Settings")
            print(f"Dataset: \033[1;34m{self.args.mosaic_data}\033[0m")
